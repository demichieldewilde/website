...
def safe_source(name, loader):
    try:
        publications = loader()
        print(f"Fetched {len(publications)} publication(s) from {name}.")
        return publications, True
    except Exception as error:
        warn(f"{name} publication sync failed: {error}")
        return [], False


def merge_all(publications):
    merged = {}
    title_index = {}
...
def main():
    manual = load_json_file(MANUAL, [])
    orcid, orcid_ok = safe_source("ORCID", from_orcid)
    arxiv, arxiv_ok = safe_source("arXiv", from_arxiv)
    fetched = [*orcid, *arxiv]
    if fetched:
        # If one source fails, keep existing committed records as a safety net so
        # a partial outage does not delete publications from the generated file.
        existing = [] if (orcid_ok and arxiv_ok) else load_existing_publications()
        publications = merge_all([*manual, *existing, *fetched])
    else:
        existing = load_existing_publications()
        if not existing:
            raise RuntimeError("No publications could be fetched and no existing assets/publications.js fallback was found.")
        warn("ORCID and arXiv both failed; keeping the existing committed publication list.")
